...
tag:
	git tag -a $(TAG_NAME) -m "ca release - $(TAG_NAME)"
...
LD_FLAGS := -ldflags "-X github.com/addoldelgrossi/redis2sheet/cmd.Version=`cat VERSION` -X github.com/addoldelgrossi/redis2sheet/cmd.BuildTime=`date -u +%Y-%m-%d_%H:%M:%S` -X github.com/addoldelgrossi/redis2sheet/cmd.GitHash=`git rev-parse HEAD` "
...
$(PLATFORMS):
	GOOS=$(os) GOARCH=$(arch) go build $(LD_FLAGS) -o 'dist/redis2sheet_$(os)-$(arch)'